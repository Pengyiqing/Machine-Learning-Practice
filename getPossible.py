import csv
with open('submission_1.csv','w',newline='') as fw1:
	writer1 = csv.writer(fw1)
#number of each distance lable of all effective coupon  
	nedlaec=[7329,40279,7210,2868,1487,1040,680,494,326,319,265,2085]
					...
					if(row1[3]==row2[0] and row2[1]!='0'):
						...
						if(row2[l]):
							possible = (int(row2[l])*nedlac[l-4])/(int(row2[1])*nedlaec[l-4])
						 	...
						 	possible = (int(row2[3])*nedlac[l-4])/(int(row2[1])*64382)
						if(possible > 1):
							possible = 1.0
