...
#商家特征：0：商家被消费次数；1：商家优惠券被领次数；2：商家优惠券被领取后核销次数；3：商家优惠券消费占总消费率；4:商家优惠券核销率；5：商家优惠券15天内核销率； 6-13：用户满0-30/30-100/100以上/打折券15天内的核销率及四种情况的比重；14-15：15天内核销情况中商家优惠券平均折率和标准差;16-17:15天内核销情况中商家到用户距离的平均距离和标准差;18-19:商家消费的平均距离和方差
f2 = pd.DataFrame(columns = [i for i in range(20)])
gs1 = f1.groupby('Merchant_id')
...
for g in gs1:
	...
	print(k)
	k = k+1
f2.to_csv('merchartFeature.csv',header = None)
